...
> 这份文件承载 [README](../README.md) 放不下的内容：机制怎么实现、有哪些开关、以及实测得到的结论。
> 面向「要在上面继续改」的读者。只想把它跑起来，看 README 就够。
>
> 相关文件：[实施计划](../plan.md) · [容器化手册](../docker/README.md) ·
> [压测报告](../loadtest/RESULTS.md) / [W3 JVM 对照](../loadtest/RESULTS-W3.md) ·
> [面试话术](interview-narrative.md)
...
依赖方向是单向的：`core ← providers ← client ← proxy`，而 `router` / `quota` / `cache` / `usage`
只依赖 `core`。所以**路由、配额、缓存、计量四块逻辑都能脱离 HTTP 单独单测**，
不靠起一个 Spring 容器来验证。
...
两个触发信号缺一不可：连续失败抓硬故障，失败率抓「轮流失败」的抖动上游——只看连续失败会漏掉后者。
...
modelgate:
  router:
    allowed-fails: 3
    cooldown-seconds: 60
    failure-window-size: 20          # 失败率窗口
    failure-rate-threshold: 0.5
    min-calls-for-rate: 10           # 样本不足时不判失败率，避免开局面就熔断
```
...
> ⚠️ **无 Redis 时多实例限流会漂**：进程内实现每个副本各持一份窗口，N 个副本就等于把额度放大 N 倍。
> 要跨副本严格限流必须用 `backend: redis`。这条是这个模块最该主动交底的边界。
...
**为什么 `/admin/**` 单独一条路径**：分析面查询不能和推理面抢线程。真实事故里，
一个两年的聚合查询把事件循环占满，导致健康探针无响应、Pod 被杀——见 `plan.md` 的避坑清单 #9。
...
`docker,loadtest` 双 profile 与 `--no-deps` 都是必须的，理由见
[docker/README.md](../docker/README.md) 4.1：
...
- 完整方法与原始数据见 [loadtest/RESULTS-W3.md](../loadtest/RESULTS-W3.md)。

> R4 与 R1–R3 **不同质**：前者是缓存命中路径的吞吐，后者是经过上游的吞吐。引用数字前要说清是哪条路径。

## 10. 容器化

整套环境（网关 + 两个 Mock + Redis）可一键起，见 [docker/README.md](../docker/README.md)：
...
- **一份路由定义同时服务宿主机与容器**。上游地址写成
  `${MODELGATE_MOCK_A_URL:http://127.0.0.1:9001}` 占位符，compose 只覆盖环境变量。
  （Spring 对 list 是整体替换而非逐项合并，所以「在 profile 里只覆盖 base-url」这条路不通。）
- **镜像内不提权、不写盘**。运行时是非 root（uid 10001）；状态在 Redis、账目在 DB、
  日志与 GC 日志都在 stdout，所以容器文件系统只读也能跑。

## 11. 构建疑难

### 为什么必须带 `-s maven-settings.xml`

本机全局 `settings.xml`（Maven 安装目录下 `conf/settings.xml`）有两个问题：

1. `localRepository` 指向 Windows 路径 `D:\...`，在 WSL 下会被当成**相对路径**，
   把依赖装进项目目录；
2. 镜像走 `http`，Maven 3.9 默认拦截不安全 HTTP 仓库，部分依赖拉不下来。

项目自带的 `maven-settings.xml` 修掉了这两点，所以构建命令统一写成
...
判断「到底是代码问题还是 IDE 缓存问题」，用这条命令即可：**不带 `-am`、纯离线**地单独构建
某个模块，它能过就说明本地仓库解析链路是通的。
